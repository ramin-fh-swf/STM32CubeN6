...
{
    /* SCK = XSPI2-Kerntakt (IC3 <- PLL1) / (DCR2.PRESCALER + 1). */
    ...
    /* Gemeinsamer Taktblock, Form und Reihenfolge mit der PSoC-Seite abgestimmt. */
    printf("\r\n--- Clock domains ---\r\n");
    printf("  %-28s = %lu Hz\r\n", "CPU",
    ...
    printf("  %-28s = %lu Hz\r\n", "NPU-RAM",
           (unsigned long) HAL_RCC_GetNPURAMSClockFreq());
    printf("  %-28s = %lu Hz\r\n", "Flash interface (SCK)",
           (unsigned long) sck_hz);

    /* Ab hier plattformeigen, ohne Entsprechung auf dem PSoC. */
    printf("\r\n--- STM32N657 only ---\r\n");
    printf("  XSPI2 SCK  = %lu.%02lu MHz [kernel=%lu Hz, DCR2.PRESCALER=%lu]\r\n",
           (unsigned long)(sck_hz / 1000000U),
           (unsigned long)((sck_hz % 1000000U) / 10000U),
           (unsigned long)xspi2_ker_hz,
           (unsigned long)prescaler);

    /* Peak = SCK * Leitungen * (DTR ? 2 : 1) / 8 -- obere Schranke, DCYC geht ab. */
    {
      ...
      printf("  XSPI2 Read = %lu lines %s, DCYC=%lu, peak=%lu.%03lu MB/s"
